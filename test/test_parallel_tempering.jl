...
@testset "ParallelTemperingMC" begin
    ...
    mktempdir() do tmpdir
        ...
        @testset "mpi" begin
            @testset for scheduler in [Carlo.MPIScheduler, Carlo.SingleScheduler]
                μs = [0, 1, 1.5, 1.7]

                job = make_test_job(
                    "$tmpdir/parallel_tempering_$scheduler",
                    10000;
                    binsize = 200,
                    thermalization = 3000,
                ...
                num_ranks = scheduler == Carlo.MPIScheduler ? length(μs) + 1 : length(μs)

                run_test_job_mpi(job; num_ranks, scheduler)
