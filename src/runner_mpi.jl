...
mutable struct MPIRunnerMaster <: AbstractRunner
    ...
    function MPIRunnerMaster(job::JobInfo, active_ranks::Integer)
        return new(active_ranks, 1, read_progress(job))
    end
end
...
function start(::Type{MPIRunnerSlave{MC}}, job::JobInfo) where {MC}
    ...
    while true
        if slave === nothing
            ...
            runner_task = RunnerTask(msg.sweeps_until_comm, 0, task_dir(job, task), 0)
        ...
        while !is_done(slave.task)
            ...
            if is_checkpoint_time(job, time_last_checkpoint) || is_end_time(job, time_start)
                break
            end
        end
